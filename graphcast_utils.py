...
def add_edge_features(graph: DGLGraph, pos: Tensor, normalize: bool = True) -> DGLGraph:
    ...
    src_pos = geospatial_rotation(src_pos, theta=theta_azimuthal, axis="z", unit="rad") # (N, 3)
    dst_pos = geospatial_rotation(dst_pos, theta=theta_azimuthal, axis="z", unit="rad") # (N, 3)
    ...
    src_pos = geospatial_rotation(src_pos, theta=theta_polar, axis="y", unit="rad") # (N, 3)
    dst_pos = geospatial_rotation(dst_pos, theta=theta_polar, axis="y", unit="rad") # (N, 3)
    ...
    disp = src_pos - dst_pos  # (N, 3)
    disp_norm = torch.linalg.norm(disp, dim=-1, keepdim=True) # (N, 1)
...
def find_subset_indices(larger, smaller):
    """
    finds the indices of all the elements in the smaller tensor in the larger tensor
    
    Parameters
    ----------
    Here (...) refers to any number of dimensions common to both tensors
    N > n
    larger : Tensor
        Tensor of shape (N, ...) containing some elements
    smaller : Tensor
        Tensor of shape (n, ...) containing some elements
    ...
    -------
    Tensor
        Tensor of shape (n, ) containing the indicies
    """
    indices = []
