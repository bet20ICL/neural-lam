...
class ERA5UKDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_name, pred_length=19, split="train", subsample_step=3,
        ...
        Required files:
        nwp*mbr*.npy - atmospheric variables
        wtr_*.npy - water coverage
        nwp_toa_downwelling_shortwave_flux_*.npy - solar radiation
        """
        super().__init__()
    ...
    def _get_sample(self, sample_name):
        ...
        full_sample = full_sample.permute(2,3,0,1) # (N_lat, N_lon, N_vars, N_levels)
        full_sample = full_sample.reshape(full_sample.shape[0]*full_sample.shape[1], -1) # (N_lat*N_lon, N_vars*N_levels)        
        return full_sample
    ...
    def __getitem__(self, idx):
        ...
        # TODO: add solar radiation to ERA5 dataset
        ...
        # TODO: add in static and forcing features later
